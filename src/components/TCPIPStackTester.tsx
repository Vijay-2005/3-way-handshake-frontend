...
import { sendPacket as apiSendPacket, resetSimulation as apiReset } from "@/lib/api";
...
export default function TCPIPStackTester() {
    ...
    const poll = async () => {
      try {
        ...
        if (Array.isArray(history) && history.length) {
          setPackets(history as any);
          setPacketCounter(history.length);
          const last = history[history.length - 1];
          setCurrentState(last.state as FSMState);
          setDataOut(last.data_out);
          setValidOut(last.valid_out);
        }
      } catch (err) {
  ...
  const sendPacket = async () => {
    ...
    // Send to backend
    let backendPacket: any = null;
    try {
      backendPacket = await apiSendPacket({ data_in: dataIn, checksum_in: checksumIn, valid_in: validIn });
    } catch (e) {
      console.error("Backend send-packet failed", e);
    }

    const newPacket: Packet = backendPacket ?? {
      id: packetCounter + 1,
      timestamp: Date.now(),
      data_in: dataIn,
    ...
    setDataOut(newPacket.data_out);
    setValidOut(newPacket.valid_out);
    setCurrentState(newPacket.state as FSMState);
